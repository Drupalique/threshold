...
## Staged world progression -- multiple acts within a single run

**Status: raw idea, unbuilt.** Raised as a "what should whole-run structure look like" question, not tied to a specific mechanic gap the way most of this doc's entries are. Today a run is one flat, continuous sequence of `RUN_MAX_DEPTH` (10) floors (`GAME_DESIGN.md` §1) -- every suit in `SUIT_DEFINITIONS` is available from floor 1 (reward/shop pools draw from all of `REWARD_SUITS` regardless of depth, `rewardGenerator.ts`), enemy difficulty ramps continuously via `EnemyDef.minFloor`/`ENEMY_COUNT_WEIGHTS_EARLY`→`LATE` (§3), and there's exactly one guaranteed boss floor at the very end (`EnemyDef.isElite`, gated to `floor >= RUN_MAX_DEPTH`, §3). This proposes carving that single descent into a small number of **acts/worlds** -- contiguous floor ranges, each with its own flavor and its own boss, rather than one undifferentiated ramp.

**Why it's worth considering:** it gives the existing floor-depth axis (already the game's one difficulty/progression knob) a second, coarser rhythm on top of it -- "getting through a world" as a legible mid-run goal distinct from "getting through a floor," the same way StS's three acts or Balatro's antes give a run internal structure and pacing beats instead of a flat climb. It also opens a natural lever for the persistent-deck suit-diversity gap this doc already returns to more than once (Suit reroll, Reward weighting -- both above): if each world spotlights a subset of suits instead of all nine being live from floor 1, a run's early deck-building has a smaller, coherent target to build toward, and later worlds can deliberately widen the pool as a form of progression in its own right, rather than diversity being purely something to fight against.

Rough shape, grounded in what already exists rather than a fresh subsystem:
- Partition `RUN_MAX_DEPTH` into N contiguous floor ranges (e.g. 3 worlds of ~3 floors each, or keep 10 and just tag ranges) -- reuses the floor number the whole engine already keys off (`RoomParams`, `EnemyDef.minFloor`, `buildRunTree`'s `floor` field), no new axis needed.
- Each world ends in a guaranteed boss floor, the same `isElite`/forced-solo shape floor 10 already has (§3) -- extend `isElite` gating from "only at `RUN_MAX_DEPTH`" to "at the end of each world," with a per-world elite roster entry instead of just one.
- Each world scopes which suits are actually in play -- `RoomParams.threatSuits`/reward pools (`REWARD_SUITS`) draw from that world's subset instead of all 9, widening as later worlds unlock (mirrors how the starter deck was recently narrowed to a handful of suits rather than the full roster, `constants.ts`'s `STARTER_ATTACK_SUIT_COUNT`/`buildStarterDeck` -- the same "start narrow, grow into the full roster" instinct applied to the whole run instead of just the opening hand).
- Optionally, a short beat between worlds (a guaranteed rest, a relic choice, or just a title card) marking the transition -- low-cost, mostly presentational.

**Open questions (raw, unresolved):**
- How many worlds, and how are `RUN_MAX_DEPTH`'s 10 floors divided among them -- fixed-size acts, or does the total run length itself change?
- Does floor numbering stay global (floor 7 reads as "world 3, floor 1") or reset per world -- affects every floor-keyed system (`minFloor` gating, `ENEMY_COUNT_WEIGHTS_EARLY/LATE`'s linear blend, §3) that currently assumes one continuous 1..10 scale.
- Does each world get its own enemy sub-roster (thematic, closer to a StS act's distinct monster set) or does the existing single roster just keep ramping through world boundaries with bosses layered on top?
- Suit unlock schedule -- fixed per world (deterministic, easy to design around) vs. seed-randomized which suits a given world spotlights (more replay variety, harder to hand-author boss kits/roster around)?
- Interacts directly with "Scale rewards with room difficulty" (above)'s optional-elite-encounter idea -- a world-boss and a mid-world optional elite are close cousins, worth designing together rather than layering independently.

**Relationship to existing systems:** `config/constants.ts`'s `RUN_MAX_DEPTH`; `engine/runTree.ts`'s `buildRunTree` (floor is already tracked per node); `config/enemies.ts`'s `EnemyDef.minFloor`/`isElite` and `roomGenerator.ts`'s `pickEnemies`/`pickEnemyCount` (§3); `engine/rewardGenerator.ts`'s `REWARD_SUITS` and `RoomParams.threatSuits` (would need a per-world suit subset threaded through, the same shape the reward-weighting entry above already threads `threatSuits` into reward odds); conceptually a much larger structural sibling of the recently-narrowed starter deck (`constants.ts`'s `STARTER_ATTACK_SUIT_COUNT`) -- both are instances of "don't show the player the whole suit roster at once, grow into it."
