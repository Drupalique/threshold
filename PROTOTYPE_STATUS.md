...
- **Feed the pool** (design doc 4.11, response to `MECHANIC_BRAINSTORM.md`'s "Feed the pool" entry): instead of claiming, a play can feed any number of same-suit hand cards face-up into the pool, growing (or seeding from zero) that suit's pile without resolving any effect -- a productive alternative for a card that doesn't match anything worth claiming right now. Costs a play like a claim does. Enemies can feed too: `IntentType` gained a `feed` step, authored per pattern (Rot Husk's cycle now ends with `feed ward +2`, a first demonstration).
...
- **Feed the pool is unplayed and unmodeled by the batch-sim bot.** The heuristic bot (`playtest-sim.ts`'s `pickBestClaim`) never issues `PLAYER_FEED` -- it always claims or passes -- so none of `PLAYTEST_FINDINGS.md`'s batch numbers reflect this mechanic yet; a bot update or human/LLM playthrough is needed before it shows up in that data. Open by feel, not by data: whether costing a full play (vs. cheaper/free) is right, whether Rot Husk's `feed ward +2` is well-tuned, and whether feeding actually changes how a dead-hand turn *feels* even though it doesn't reduce the dead-hand turn *count* (see design doc 4.11's open questions).
